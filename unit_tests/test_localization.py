...
import numpy as np
from vtr_regression_testing.localization_chain import LocalizationChainFactory, LocalizationIterator
import argparse
...
def graph(graph_path):
    factory = LocalizationChainFactory(graph_path)
    return factory.buildGraph()

@pytest.fixture
def baseline(baseline_path):
    factory = LocalizationChainFactory(baseline_path)
    return factory.buildGraph()
...
def test_pose_error(graph, baseline):
    ...
    for (v1, _), (v2, _) in zip(LocalizationIterator(baseline.root), LocalizationIterator(graph.root)):
...
if __name__ == '__main__':
    ...
    base_factory = LocalizationChainFactory(baseline_graph_dir)
    baseline_graph = base_factory.buildGraph()

    can_factory = LocalizationChainFactory(candidate_graph_dir)
    ...
    for (v1, _), (v2, _) in zip(LocalizationIterator(baseline_graph.root), LocalizationIterator(candidate_graph.root)):
        pose_err.append(v1.T_v_w.r_ba_ina()-v2.T_v_w.r_ba_ina())
        ang_err.append(v1.T_v_w.C_ba() @ v2.T_w_v.C_ba())
        t.append(v1.stamp / 1e9)
